...
^#x::
	...
	If (sURL != "")
		{
		Reg_1:=Format(sURL)
		Reg_2:=REGEXmatch(Reg_1 , "(?:https?:\/{2})?(?:w{3}\.)?youtu(?:be)?\.(?:com|be)(?:\/watch\?v=|\/)([^\s&]+)")
		Length := StrLen(Reg_2)
		If (Length < 5)
			{
			RegExMatch(Reg_1, "^.*?(?:v|list)=(.*?)(?:&|$)" , Reg_3)
			Reg_2:=Reg_3
			Length := StrLen(Reg_2)
			}
		If (Length < 5)
			TRAYTIP, Youtube, Error
		If (Reg_2 != "")
			{
			YT_iD := RegExreplace(Reg_2,")(index=)[0-9]*","")
			RunWait, %comspec% /c "youtube-dl.exe %YT_iD% -x --extract-audio" 			
			TRAYTIP, YouTube, Audio Extracted to Path,
			}
		}
